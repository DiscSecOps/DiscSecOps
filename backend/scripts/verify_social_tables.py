...
async def verify_tables():
    ...
    async for session in get_db():
        ...
        result = await session.execute(
            text("SELECT column_name FROM information_schema.columns WHERE table_name = 'circle_members' AND column_name = 'is_admin'")
        )
        if not result.scalar():
        ...
        result = await session.execute(
            text("SELECT column_name FROM information_schema.columns WHERE table_name = 'circle_members' AND column_name = 'is_moderator'")
        )
        if not result.scalar():
