...
def _(page: Page, datatable):
    ...
    for expected in expected_circles:
        ...
        try:
            ...
            errors.append(
                f"❌ {expected['circle_name']} Role Error: {e.args[0].split('Call log:')[0].strip()}"
            )
        ...
        try:
            ...
            errors.append(
                f"❌ {expected['circle_name']} Badge Error: {e.args[0].split('Call log:')[0].strip()}"
            )
    ...
    if errors:
        ...
        raise AssertionError(
            f"Dashboard UI validation failed with {len(errors)} errors:\n\n{error_summary}"
        )
