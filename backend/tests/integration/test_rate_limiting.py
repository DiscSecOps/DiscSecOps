...
class TestRegisterRateLimiting:
    ...
    @pytest.mark.asyncio
    async def test_register_first_request_not_rate_limited(self, client: AsyncClient) -> None:
        """First request to /register is never rate-limited."""
        response = await client.post("/api/v1/auth/register", json=_register_payload("first"))
        # 201 = created, 400 = validation/duplicate — either way NOT 429
        assert response.status_code in (201, 400, 422)

    @pytest.mark.asyncio
    async def test_register_rate_limited_after_threshold(self, client: AsyncClient) -> None:
        ...
        response = await client.post("/api/v1/auth/register", json=_register_payload("overflow"))
        assert response.status_code == 429

    @pytest.mark.asyncio
    async def test_register_429_returns_json_body(self, client: AsyncClient) -> None:
        ...
        response = await client.post("/api/v1/auth/register", json=_register_payload("json_over"))
        assert response.status_code == 429
        data = response.json()
        assert "error" in data

    @pytest.mark.asyncio
    async def test_register_exactly_at_limit_still_succeeds(self, client: AsyncClient) -> None:
        ...
        response = await client.post("/api/v1/auth/register", json=_register_payload("boundary_4"))
        assert response.status_code in (201, 400, 422)  # NOT 429

    @pytest.mark.asyncio
    async def test_register_reset_storage_clears_counters(self, client: AsyncClient) -> None:
        ...
        blocked = await client.post("/api/v1/auth/register", json=_register_payload("rst_over"))
        assert blocked.status_code == 429

        # Reset — counters are now zero
        limiter._storage.reset()

        response = await client.post("/api/v1/auth/register", json=_register_payload("rst_after"))
        assert response.status_code in (201, 400, 422)  # NOT 429
...
class TestLoginRateLimiting:
    ...
    @pytest.mark.asyncio
    async def test_login_first_request_not_rate_limited(self, client: AsyncClient) -> None:
    ...
    @pytest.mark.asyncio
    async def test_login_429_returns_json_body(self, client: AsyncClient) -> None:
...
class TestRateLimitingIsolation:
    ...
    @pytest.mark.asyncio
    async def test_register_and_login_counters_are_independent(self, client: AsyncClient) -> None:
    ...
    @pytest.mark.asyncio
    async def test_health_endpoints_never_rate_limited(self, client: AsyncClient) -> None:
