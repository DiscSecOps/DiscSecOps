...
// TODO: Ask the team if we really want to test redirection behaviour here 
/*
test('Login page redirection to Register @auth', async ({ page }) => {
  await page.goto('http://localhost:3000/register');
  ...
  // 2. Fill in the form with valid data
  await page.locator('#email').fill('newuser2@example.com');
  await page.locator('#username').fill('testuser2');
  await page.locator('#password').fill('Test123!');
  await page.locator('#confirmPassword').fill('Test123!');

  // 3. "The Create Account" should now be enabled
  await expect(page.getByRole('button', { name: 'Create Account'})).toBeEnabled();

  // 4. Click the "Create Account" button and await redirection to Login page
  await page.getByRole('button', { name: 'Create Account' }).click();
  
  // 5. Check that user has been redirected to the Login page
  await expect(page).toHaveURL(/login/);

  // 6. The account creation success message should appear
  await expect(page.getByText('Account created for testuser2! You can now login.')).toBeVisible();
});
*/